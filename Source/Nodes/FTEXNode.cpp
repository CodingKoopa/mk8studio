...
FTEXNode::FTEXNode(FTEX* ftex, QObject* parent)
    : Node(parent), m_ftex(ftex), m_sections_container(nullptr), m_header_loaded(false),
...
ResultCode FTEXNode::LoadAttributeArea()
{
  ...
  header_attributes_model->setItem(row, 1,
                                   new QStandardItem(QString::number(m_ftex_header->width)));
  row++;
